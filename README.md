...
.venv/bin/python app.py    # 服务就绪后自动打开 http://127.0.0.1:5731
```

VS Code 打开此工作区时会自动启动同一个本地服务并打开浏览器；也可以用
**Ctrl+Shift+P → Tasks: Run Task → 启动语料库网页** 手动重启。该入口是本地模式，
上传和删除可用。该任务使用 VS Code 当前选中的 Python 解释器；若该环境未装依赖，
先运行上面的安装命令，或在状态栏切换解释器。
